...
public class ChatController {
    ...
    public R sendPrivateMessage(@RequestBody MessagesBo messages){
        ...
        AiMessage aiMessage = messageService.sendPrivateMessage(messages, userId);
        if(messages.getReceiveId() == 0){
            oneBotClient.sendPrivateMessage(aiMessage.getId().toString(), userId.toString(), messages.getMessages());
        }
        return R.ok();
    }
